...
export default function CaseStudiesPage() {
  ...
  return (
    <main className="text-white h-auto md:min-h-screen px-6 md:px-16 py-16 bg-[#0A0A0A]">
      ...
      {/* GRID VIEW */}
      {view === "grid" && (
        <div className="grid grid-cols-1 md:grid-cols-2 2xl:grid-cols-2 4xl:grid-cols-3  gap-8 mt-12 pb-15">
          {filteredCases.map((item) => (
            <Link href={`/case-studies/${item.slug}`}>
              <div key={item.id} className="group cursor-pointer">
                ...
                <h3 className="mb-3 text-[25px] leading-[36.5px] tracking-[-0.16px] text-white flex items-center gap-x-[30px]">
                  {item.title}{" "}
                  <Image
                    className="rotate-[229deg]"
                    src={DownArrow}
                    alt="BBT Down Arrow"
                    width={18}
                    height={18}
                ...
                <span className="text-[13px] leading-[16.5px] tracking-[-0.16px] text-white/65">
                  {" "}
                  {item.description}
                </span>
              </div>
            ...
            <Link href={`/case-studies/${item.slug}`}>
              ...
              >
                <div className="flex gap-3  md:gap-6 items-center w-[100%] md:w-[45%] list-details-1">
                  ...
                  <h3 className="font-normal text-[26px] leading-[39px] tracking-[-0.52px] ">
                    {item.title}
                  </h3>
                </div>
                ...
                <div className="flex gap-3   md:gap-10 text-sm text-neutral-400 mt-2 md:mt-0 w-[100%] md:w-[55%] flex justify-between list-details-2">
                  ...
                  <span className="font-medium text-[12px] leading-[18px] tracking-[-0.16px] text-white/55">
                    {item.services}
                  </span>
                  <span className="font-medium text-[11px] leading-[16px] tracking-[-0.16px] text-white/65">
                    {item.year}
                  </span>
                </div>
              </div>
